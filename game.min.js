...
ig.baked=true;ig.module('game.settings.settings').defines(function(){Settings=ig.Class.extend({easyGrid:[{x:252,y:176},{x:456,y:176},{x:660,y:176},{x:252,y:416},{x:456,y:416},{x:660,y:416}],testGrid:[{x:252,y:176},{x:456,y:176},{x:660,y:176},{x:252,y:416},{x:456,y:416},{x:660,y:416}],mediumGrid:[{x:192,y:96},{x:360,y:96},{x:528,y:96},{x:696,y:96},{x:192,y:296},{x:360,y:296},{x:528,y:296},{x:696,y:296},{x:192,y:496},{x:360,y:496},{x:528,y:496},{x:696,y:496}],hardGrid:[{x:124,y:20},{x:256,y:20},{x:388,y:20},{x:520,y:20},{x:652,y:20},{x:784,y:20},{x:124,y:196},{x:256,y:196},{x:388,y:196},{x:520,y:196},{x:652,y:196},{x:784,y:196},{x:124,y:376},{x:256,y:376},{x:388,y:376},{x:520,y:376},{x:652,y:376},{x:784,y:376},{x:124,y:552},{x:256,y:552},{x:388,y:552},{x:520,y:552},{x:652,y:552},{x:784,y:552}],animalCards:['EntityBird','EntityCat','EntityCrab','EntityDog','EntityDuck','EntityFish','EntityGiraffe','EntityLion','EntityPenguin','EntityPig','EntitySnake','EntitySpider','EntityTurtle'],testCards:['EntityBird','EntityCat','EntityCrab'],difficulty:{test:3,easy:3,medium:6,hard:12}});});
...
ig.baked=true;ig.module('game.entities.baseDifficultyButton').requires('impact.entity').defines(function(){EntityBaseDifficultyButton=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/easybutton.png',216,72),size:{x:216,y:72},clickOnMe:function(){return(ig.input.mouse.y>this.pos.y&&ig.input.mouse.y<this.pos.y+this.size.y)&&(ig.input.mouse.x>this.pos.x&&ig.input.mouse.x<this.pos.x+this.size.x);},init:function(x,y,settings){this.addAnim('idle',1,[0]);this.addAnim('clicked',1,[1],true);this.currentAnim=this.anims.idle;this.parent(x,y,settings);},update:function(){if(this.clickOnMe()){if(ig.input.pressed('click')){this.currentAnim=this.anims.clicked;}else if(ig.input.released('click')){GAME_STATE.difficultySetting=this.difficultySetting;this.currentAnim=this.anims.idle;ig.system.setGame(SceneSelectScreen);}}
this.parent();}});});
...
ig.baked=true;ig.module('game.entities.animals.bird').requires('game.entities.card').defines(function(){EntityBird=EntityCard.extend({animSheet:new ig.AnimationSheet('media/cards/card.png',120,195),underCard:new ig.Image('media/cards/animals/bird.png'),value:"EntityBird"});});
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.settings.settings','game.utility','game.levels.camping','game.levels.loadingscreen','game.levels.sceneselect','game.entities.animals.bird','game.entities.animals.cat','game.entities.animals.crab','game.entities.animals.dog','game.entities.animals.duck','game.entities.animals.fish','game.entities.animals.giraffe','game.entities.animals.lion','game.entities.animals.penguin','game.entities.animals.pig','game.entities.animals.snake','game.entities.animals.spider','game.entities.animals.turtle').defines(function(){GAME_STATE={difficultySetting:"",sceneSetting:""};MyGame=ig.Game.extend({settings:new Settings(),utility:new Utility(),matchOne:"",matchTwo:"",font:new ig.Font('media/04b03.font.png'),init:function(){this.currentDeck=this.settings.animalCards;console.log(GAME_STATE);this.loadLevel(LevelCamping);this.createBoard(this.settings.difficulty[GAME_STATE.difficultySetting]);},update:function(){if(this.cardCount===0){this.createBoard(this.settings.difficulty[GAME_STATE.difficultySetting]);}
this.parent();},createBoard:function(cardCount){this.cardCount=cardCount*2;var shuffledCards=this.getRandomCards(cardCount);var animalCardsCopy=this.utility.shuffle(shuffledCards.concat(shuffledCards));var grid=this.settings[GAME_STATE.difficultySetting+'Grid'];for(var i=0;i<grid.length;i++){this.spawnEntity(animalCardsCopy[i],grid[i].x,grid[i].y);}},getRandomCards:function(count){return this.utility.shuffle(this.currentDeck).slice(0,count);}});StartScreen=ig.Game.extend({instructText:new ig.Font('media/04b03.font.png'),background:new ig.Image('media/LoadingScreen.png'),init:function(){ig.input.bind(ig.KEY.MOUSE1,'click');this.loadLevel(LevelLoadingscreen);},draw:function(){this.parent();var x=ig.system.width/2,y=ig.system.height-10;this.instructText.draw('A BRAIN TEASER',x+50,y-220,ig.Font.ALIGN.LEFT);this.instructText.draw('BY: JOEL TADDEI',x+30,y-180,ig.Font.ALIGN.LEFT);this.instructText.draw('  & KYLE LAWSON',x+30,y-140,ig.Font.ALIGN.LEFT);}});SceneSelectScreen=ig.Game.extend({instructText:new ig.Font('media/04b03.font.png'),init:function(){ig.input.bind(ig.KEY.MOUSE1,'click');this.loadLevel(LevelSceneselect);},draw:function(){this.parent();var x=ig.system.width/2,y=ig.system.height-10;this.instructText.draw('SELECT A SCENE',x+50,y-220,ig.Font.ALIGN.LEFT);}});EasyGame=MyGame.extend({difficulty:"easy"});MediumGame=MyGame.extend({difficulty:"medium"});HardGame=MyGame.extend({difficulty:"hard"});ig.main('#canvas',StartScreen,60,1024,768,1);});